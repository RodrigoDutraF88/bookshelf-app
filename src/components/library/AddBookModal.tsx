...
const BarcodeScanner = dynamic(
  () => import("~/components/library/BarcodeScanner").then((m) => ({ default: m.BarcodeScanner })),
  { ssr: false },
);
...
export function AddBookModal({ isOpen, onClose }: Props) {
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) { if (e.key === "Escape" && isOpen && !showScanner) onClose(); }
    document.addEventListener("keydown", onKey);
    return () => document.removeEventListener("keydown", onKey);
  }, [isOpen, onClose, showScanner]);
  ...
  return (
    <>
      ...
      {showScanner && (
        <BarcodeScanner
          onScan={(isbn) => { void handleIsbnScan(isbn); }}
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-2 px-6 pt-4">
            <div className="flex gap-1" role="tablist" aria-label="Add book method">
          ...
          {tab === "search" && (
            <div className="flex flex-col gap-3 p-6">
              ...
              {searchErr && (
                <p style={{ fontSize: 13, color: "var(--color-danger)", textAlign: "center", padding: "16px 0" }}>
                  Search failed. Check your connection and try again.
                </p>
              )}

              {!searching && !searchErr && query.length >= 3 && results.length === 0 && (
                  ...
                  {results.map((book) => (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 13, fontWeight: 600, color: "var(--color-text-primary)", fontFamily: "var(--font-display)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginBottom: 2 }}>
                          {book.title}
                        </p>
              ...
              {!query && (
                <p style={{ fontSize: 12, color: "var(--color-text-muted)", textAlign: "center", padding: "20px 0" }}>
                  Search by title or author · or tap <strong>Scan ISBN</strong> to use your camera
                </p>
              )}
            </div>
          )}
          ...
          {tab === "manual" && (
            <form onSubmit={handleSubmit} className="flex flex-col gap-5 p-6">
              <div className="flex gap-4 items-start">
                ...
                <div className="flex-1 flex flex-col gap-3">
                  <Field label="Title" required>
                    <input ref={firstInputRef} type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="The Name of the Wind" required className={inputClass} />
                  </Field>
                  <Field label="Author" required>
                    <input type="text" value={author} onChange={(e) => setAuthor(e.target.value)} placeholder="Patrick Rothfuss" required className={inputClass} />
                  </Field>
                </div>
              </div>
              ...
              <Field label="Description">
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="A brief synopsis or your own notes…" rows={3} className={inputClass + " resize-none"} />
              </Field>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Published year">
                  <input type="number" value={publishedYear} onChange={(e) => setPublishedYear(e.target.value)} placeholder="2007" min={0} max={new Date().getFullYear()} className={inputClass + " font-mono"} />
                </Field>
              </div>

              <Field label="ISBN" hint="Optional">
                <input type="text" value={isbn} onChange={(e) => setIsbn(e.target.value)} placeholder="978-0-7564-0407-1" className={inputClass + " font-mono"} />
              </Field>
              ...
              <div className="flex gap-3 justify-end pt-1">
                <button type="button" onClick={handleClose} className="px-4 py-2 rounded-lg text-sm font-medium" style={{ backgroundColor: "var(--color-surface-raised)", color: "var(--color-text-secondary)", border: "1px solid var(--color-border)" }}>
                  Cancel
                </button>
                <button type="submit" disabled={createBook.isPending || !title.trim() || !author.trim()} className="px-5 py-2 rounded-lg text-sm font-medium disabled:opacity-40" style={{ backgroundColor: "var(--color-accent)", color: "#0F0F0F" }}>
...
function CameraIcon() { return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>; }